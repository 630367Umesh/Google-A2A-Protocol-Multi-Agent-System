...
class WriterAgentA2A(GoogleA2AServer):
    ...
    async def handle_create_article(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            article_text = f"✍️ Article by Alex Writer\n{'='*60}\n{response.text}"
            # Save to PDF with formatting and timestamped filename
            pdf_path = save_article_to_pdf(
    ...
    async def handle_create_marketing_copy(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # Save to PDF with formatting and timestamped filename
            pdf_path = save_article_to_pdf(
                title=f"Marketing Copy: {product_service}",
                content=response.text
            )
            return {
